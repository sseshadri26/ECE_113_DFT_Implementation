...
def plot_dft_magnitude_angle(frequency_axis : np.array, f_signal : np.array, fs : int=1, format : str=None):
    ...
    if(format == "ZeroPhase"):
        ...
        # Set the phase to zero if any point in the DFT is less than some very small value (e.g. 1e-4)
        # clone f_signal into f_signal_fixed
        # f_signal_fixed[np.where(np.abs(f_signal_fixed) < 1e-4)] = 0
        
        f_signal_fixed = np.where(np.abs(f_signal) < 1e-4, 0, f_signal)
        
        ax1.set_ylabel("Magnitude")
        ax1.stem(frequency_axis, np.abs(f_signal_fixed))
        ax2.set_ylabel("Phase (radians)")
        ax2.stem(frequency_axis, np.angle(f_signal_fixed))
        ax2.set_xlabel("Frequency Bins")
        ax2.set_ylim((-np.pi, np.pi))
        
    if(format == "Normalized"):
        #Plot DFT such that the Frequency axis is normalized between [0, 1].
        ax1.set_ylabel("Magnitude")
        ax1.stem(frequency_axis/N, np.abs(f_signal))
        ax2.set_ylabel("Phase (radians)")
        ax2.stem(frequency_axis/N, np.angle(f_signal))
        ax2.set_xlabel("Normalized Frequency Bins")
        ax2.set_ylim((-np.pi, np.pi))
  

    if(format == "Centered_Normalized"):
        #  Plot DFT such that the Frequency axis is normalized between [-0.5, 0.5]
        # Hint: This will require shifting indices around.
        #the numpy fftshift function does exactly this.
        #also shift the labels on the x-axis to reflect the new frequency range.
        ax1.set_ylabel("Magnitude")
        ax1.stem(np.fft.fftshift(frequency_axis)/N -0.5, np.abs(f_signal))
        ax2.set_ylabel("Phase (radians)")
        ax2.stem(np.fft.fftshift(frequency_axis)/N -0.5, np.angle(f_signal))
        ax2.set_xlabel("Normalized Frequency Bins")
        ax2.set_ylim((-np.pi, np.pi))
    ...
    if(format == "Centered_Original_Scale"):
        ...
        ax1.set_ylabel("Magnitude")
        ax1.stem(np.fft.fftshift(frequency_axis)*fs/N -fs/2, np.abs(f_signal))
        ax2.set_ylabel("Phase (radians)")
        ax2.stem(np.fft.fftshift(frequency_axis)*fs/N -fs/2, np.angle(f_signal))
...
def convolve_signals(x_signal : np.array, y_signal : np.array) -> np.array:
    ...
    z_signal_dft = np.multiply(x_signal_dft, y_signal_dft)
